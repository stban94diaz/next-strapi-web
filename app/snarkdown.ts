...
export default function mdToHtml(md: string, prevLinks: { [key: string]: string } = {}): string {
	const tokenizer = /((?:^|\n+)(?:\n---+|\* \*(?: \*)+)\n)|(?:^``` *(\w*)\n([\s\S]*?)\n```$)|((?:(?:^|\n+)(?:\t|  {2,}).+)+\n*)|((?:(?:^|\n)([>*+-]|\d+\.)\s+.*)+)|(?:!\[([^\]]*?)\]\(([^)]+?)\))|(\[)|(\](?:\(([^)]+?)\))?)|(?:(?:^|\n+)([^\s].*)\n(-{3,}|={3,})(?:\n+|$))|(?:(?:^|\n+)(#{1,6})\s*(.+)(?:\n+|$))|(?:`([^`].*?)`)|(  \n\n*|\n{2,}|__|\*\*|[_*]|~~)/gm;
	const context: string[] = [];
	let out = '';
	const links = prevLinks;
	let last = 0;
	let chunk: string, prev: string, token: RegExpExecArray | null, inner: string, t: string | undefined;
	...
	while ((token = tokenizer.exec(md))) {
		...
		// Headings:
			...
			chunk = `<${t}>${mdToHtml(token[12] || token[15], links)}</${t}>`;
		}
		// `code`:
		else if (token[16]) {
